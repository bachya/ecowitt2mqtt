...
from ecowitt2mqtt.helpers.meteo import PressureConverter, WindConverter

DEFAULT_TEMP_UNIT = "f"
DEFAULT_UNIQUE_ID = "default"

NON_DATA_KEYS = ["PASSKEY", "dateutc", "freq", "model", "stationtype"]
...
class DataProcessor:
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize."""
        self._data = {k: v for k, v in payload.items() if k not in NON_DATA_KEYS}
    ...
    def _parse_data(self, keys: List[Tuple[str, str]]) -> Dict[str, Any]:
        """Generate parsed data."""
        data = {}

        for original_key, new_key in keys:
            if original_key not in self._data:
                continue

            original_value = float(self._data.pop(original_key))
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        for original_key, new_key in [
            ...
            pressure_data[new_key] = converter.pressure
        ...
        for original_key, new_key in [
            ...
            original_value = float(self._data.pop(original_key))
            converter = WindConverter(
                original_value,
                input_unit_system=self._input_unit_system,
                output_unit_system=self._output_unit_system,
            )
            wind_data[new_key] = converter.pressure
...
#             self,
#             "_data",
#             {k: v for k, v in self._input.items() if k not in NON_DATA_KEYS},
#         )
